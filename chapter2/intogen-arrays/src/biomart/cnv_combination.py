...
__COLUMN_NAMES = [
	"gain_n", "gain_z_score", "gain_p_value",
	"loss_n", "loss_z_score", "loss_p_value"]
...
def main():
	...
	for name in table_infixs:
		...
		cursor.execute("""
			CREATE TABLE IF NOT EXISTS cmb_{0}_cna (
			  {0}_id int(11) NOT NULL,
			  icdo_id int(11) NOT NULL,
			  gain_n int(11) DEFAULT NULL,
			  gain_zscore double DEFAULT NULL,
			  gain_pvalue double DEFAULT NULL,
			  loss_n int(11) DEFAULT NULL,
			  loss_zscore double DEFAULT NULL,
			  loss_pvalue double DEFAULT NULL,
			  PRIMARY KEY ({0}_id,icdo_id),
			  KEY icdo (icdo_id),
			  CONSTRAINT cmb_{0}_cna_{0}_id FOREIGN KEY ({0}_id) REFERENCES ent_{0} ({1}id),
			  CONSTRAINT cmb_{0}_cna_icdo_id FOREIGN KEY (icdo_id) REFERENCES ent_icdo (id)
	...
	for id_type, eid in id_port:
		e = em.find(eid, types.CNV_COMBINATION)
		if e is None:
			log.error("{} not found: {}".format(types.CNV_COMBINATION, eid))
			continue

		if "results_file" not in e:
			log.error("{} [{}] without results file.".format(types.CNV_COMBINATION, eid))
			continue
		...
		ib = BatchInsert(cursor, "cmb_{}_cna".format(table_infix),
				["{}_id".format(table_infix), "icdo_id",
					"gain_n", "gain_zscore", "gain_pvalue",
					"loss_n", "loss_zscore", "loss_pvalue"],
				insert_size)
