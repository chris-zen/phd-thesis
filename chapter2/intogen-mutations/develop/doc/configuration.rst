...
In the case of the web server, the workflow configuration has to be defined through the enviroment variables **WOK_CONF_PATH**, **WOK_CONF_FILES** and **WOK_CONF_ARGS**. For example, to specify that the configuration file *conf/specific_configuration.conf* has to be loaded you can run the server as::

	$ export WOK_CONF_PATH="$(pwd)/conf"
	$ export WOK_CONF_FILES="specific_configuration"
	$ ./run service

Note that you can skip the *.conf* extension.

The configuration specific to the web server can be defined with the enviroment variables WEB_CONF_PATH and WEB_CONF_FILES. For example::

	$ export WEB_CONF_PATH="$(pwd)/conf"
	$ export WEB_CONF_FILES="analytics"
	$ ./run service

This will load the file *./conf/analytics.cfg* as the web server configuration.

You will find further documentation on how to configure the workflows and the web server:
