...
def list_projects(log, path, base_path=None):
	#log.debug("  {}".format(os.path.relpath(path, base_path or path)))
			...
			try:
				with open(path, "r") as f:
					...
					if isinstance(project, dict):
						...
						log.warn("Discarding malformed file {}".format(os.path.relpath(path, base_path)))
			except:
				log.warn("Unexpected error reading file {}".format(os.path.relpath(path, base_path)))
