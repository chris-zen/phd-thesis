...
class MaDb(object):
	def __init__(self, path, part_size=None, source_time=None):
		self.path = path
	...
	def open(self):
		...
		try:
			c.execute("SELECT db_version, source_time FROM meta")
			res = c.fetchone()
			db_version, source_time = res
			source_time = datetime.strptime(source_time, DATE_FMT)
		except:
			self.create()
			db_version = DB_VERSION
			source_time = self.source_time

		if db_version != DB_VERSION or (self.source_time is not None and self.source_time != source_time):
			self.drop()
			self.create()
			db_version = DB_VERSION
		...
		c.execute("SELECT creation_time, source_time, part_size FROM meta")
		res = c.fetchone()
		if res is None:
			raise Exception("unexpected error: meta table is empty")

		creation_time, source_time, part_size = res

		self.creation_time = datetime.strptime(creation_time, DATE_FMT)
		self.source_time = datetime.strptime(source_time, DATE_FMT)
	...
	def create(self):
		...
		if self.source_time is None:
			raise Exception("source_time not defined")

		c.execute("CREATE TABLE meta (db_version, creation_time, source_time, part_size INTEGER)")
		c.execute("INSERT INTO meta VALUES (?,?,?,?)",
			(DB_VERSION, datetime.now().strftime(DATE_FMT), self.source_time.strftime(DATE_FMT), self.part_size))
	...
	def __get_id(self, c, var_name, value):
		...
		if var_name in self.__id_cache:
			#last_value, last_id = self.__last_id[var_name]
			#if value == last_value:
			#	return last_id
		...
		var_id = c.lastrowid
		"""
		c.execute("SELECT {0}_id FROM {0} WHERE name=?".format(var_name), (value,))
		res = c.fetchone()
		if res is None:
			c.execute("INSERT INTO {0} (name) VALUES (?)".format(var_name), (value,))
			var_id = c.lastrowid
			...
			var_id = res[0]
		"""
		#self.__last_id[var_name] = (value, var_id)
	...
	def get(self, chr, start, ref, alt, var_id=None):
		part = start / self.part_size
		...
		return MaResult(var_id=var_id, chr=chr, start=start, ref=ref, alt=alt, uniprot=uniprot, fi_score=fi_score)
