...
from intogensm.paths import get_quality_variants_path, get_quality_oncodrivefm_path, get_quality_oncodriveclust_path
			...
			for wi in case.workitems(task.cname):
				if wi.state == FAILED:
					wi_exceptions += [dict(
						...
						exitcode=wi.result["exitcode"],
						exception=wi.result["exception"],
						trace=wi.result["trace"])]
			exceptions += [dict(
				cname=task.cname[8:] if task.cname.startswith("project.") else task.cname,
...
def download(case_id):
	...
	case_path = case.properties["path"]
	dest_path = os.path.join(case_path, "results.zip")
	if not os.path.exists(dest_path):
		flash("Sorry, there were an internal error and the results are not available", "error")
		app.logger.error("[{}] results.zip not available for case {} at {}".format(current_user.nick, case.id, case_path))
		return redirect(url_for(".index"))
	...
	return send_file(
				...
				as_attachment=True,
				attachment_filename="IntOGenMutations_{}.zip".format(case.name))
